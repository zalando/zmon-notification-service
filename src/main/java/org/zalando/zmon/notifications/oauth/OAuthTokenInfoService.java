...
public class OAuthTokenInfoService implements TokenInfoService {
    ...
    // oauthtoken -> uid
    private final Cache<String, String> tokenCache = CacheBuilder.newBuilder().expireAfterWrite(600, TimeUnit.SECONDS)
            .build();
    ...
    private Optional<String> queryCacheOrServer(String oauthToken) {
        try {
            return Optional.ofNullable(tokenCache.get(oauthToken, () -> queryOAuthServer(oauthToken)));
        } catch (Exception e) {
    ...
    // curl --insecure -H 'Authorization: Bearer
    // de871723-b003-485d-9339-320bb0d3a99d'
    // https://zmon-notification-service-3.stups.zalan.do/api/v1/mobile/alerts
    ...
    public String toString() {
        return MoreObjects.toStringHelper(this).add("serviceUrl", serviceUrl).toString();
    }
}
